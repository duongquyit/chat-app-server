...
io.on("connection", (socket) => {
  ...
  const userExistIndex = listUsersConnection.findIndex(
    ({ uid }) => uid == userConnect.uid
  );
  if (userExistIndex == -1) {
  ...
  socket.on("notificationReceiveMessage", ({ sender, receiver }) => {
    ...
    if (index == -1) {
      ...
      if (!listRooms[index].includes(socket.id)) {
        socket
          .to(`${receiver.socket}`)
          .emit("notificationNewMessage", { sender, receiver });
      }
    }
  });
  ...
  socket.on("typingText", (roomKey) => {
    listUsersTyping.push(socket.handshake.auth);
    socket.broadcast.emit("listUsersTyping", listUsersTyping, roomKey);
  });

  socket.on("stopTyping", () => {
    const index = listUsersTyping.findIndex(
      ({ uid }) => uid == socket.handshake.uid
    );
    listUsersTyping.splice(index, 1);
    socket.broadcast.emit("listUsersTyping", listUsersTyping);
  });

  socket.on("disconnect", () => {
    console.log(`${userConnect.socket} is disconnect`);
    const userIndex = listUsersConnection.findIndex(
      (user) => user.uid === userConnect.uid
    );
